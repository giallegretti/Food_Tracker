...
function buildProjection(
  ...
  for (let week = 0; week < 104; week++) {
    ...
    // Meta = basal (come no basal) ou TDEE − déficit fixo. Não deixa a ingestão
    // cair abaixo do mínimo; o déficit que move a perda é TDEE − ingestão.
    const target = targetMode === "basal" ? bmr : tdee - deficitKcal;
    const intake = Math.max(target, minIntake);
    const dailyDeficit = Math.max(tdee - intake, 0);
...
export default function PesoPage() {
  ...
  const [tab, setTab] = useState<"registro" | "projecao" | "curvas">(
    "registro"
  );
